...
proc initWithDefaultConfigFiles*(
    ...
    ## Initializes the Command with the default config files, if existing, which are:
    ## - ./.highlight.json
    let appConfigFilepath = "." & os.extractFilename(os.getAppFilename()) & ".json"
    if os.existsFile(appConfigFilepath):
        self.initWithConfigFile(appConfigFilepath)
    return self
...
proc initWithCliArgs*(
    ...
    ## Returns an Array of Error-Strings to present this the User.
...
proc hasConfigErrors*(self: HighlightCommand): bool =
    ## Gibt an ob Konfigurationsfehler vorliegen, welche durch fehlerhafte Kommandozeilenargumente
    ## oder fehlerhafte Config-Dateien entstehen können. Wenn solche Fehler vorliegen, wird das
    ## Execute-Commando nicht ausgeführt und stellt diese dem Nutzer dar.
